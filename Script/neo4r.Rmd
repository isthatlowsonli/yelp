...
Since the path is too large for directed import to `igraph` object via `neo4r`, I decide to export:
1) node list
2) relationship list 
...
```{r}
transitivity(graph_object, type="global")  # net is treated as an undirected network

transitivity(as.undirected(graph_object, mode="collapse")) # same as above
...
hist(deg, breaks=1:vcount(graph_object)-1, main="Histogram of node degree")
```

```{r}
deg.dist <- degree_distribution(graph_object, cumulative=T, mode="all")
